...
def _exterior_walls(doc):
    ...
    w = w.cut(wcut(AX + A_W - 4.0 * M, AY - wt3 / 2, SLAB_T,
                   3.30 * M, 2.08 * M, wt3))
    # Окно кухни (запад)
    w = w.cut(wcut(AX - wt3 / 2, AY + 1.5 * M, SLAB_T + 0.9 * M,
                   wt3, 1.5 * M, 1.2 * M, "x"))
    # Окно север
    ...
    # --- Блок B (спальни) ---
    # Окна восток
    w = w.cut(wcut(BX + B_W - wt3 / 2, BY + B_D - 2.5 * M,
                   SLAB_T + 0.5 * M, wt3, 1.4 * M, 1.8 * M, "x"))
    w = w.cut(wcut(BX + B_W - wt3 / 2, BY + 3.8 * M,
                   SLAB_T + 0.5 * M, wt3, 1.2 * M, 1.5 * M, "x"))
    w = w.cut(wcut(BX + B_W - wt3 / 2, BY + 0.8 * M,
                   SLAB_T + 0.5 * M, wt3, 1.2 * M, 1.5 * M, "x"))
    # Окно юг блок B
    w = w.cut(wcut(BX + 1.5 * M, BY - wt3 / 2, SLAB_T + 0.5 * M,
                   1.0 * M, 1.5 * M, wt3))
    # Маленькие окна санузлов
    w = w.cut(wcut(BX + B_W - wt3 / 2, BY + B_D - 4.5 * M,
                   SLAB_T + 1.2 * M, wt3, 0.6 * M, 0.6 * M, "x"))
    w = w.cut(wcut(BX + B_W - wt3 / 2, BY + B_D - 6.5 * M,
                   SLAB_T + 1.2 * M, wt3, 0.6 * M, 0.6 * M, "x"))
    # Входная дверь (запад блока B)
    w = w.cut(wcut(BX - wt3 / 2, BY + 8 * M, SLAB_T,
                   wt3, 1.0 * M, 2.2 * M, "x"))
